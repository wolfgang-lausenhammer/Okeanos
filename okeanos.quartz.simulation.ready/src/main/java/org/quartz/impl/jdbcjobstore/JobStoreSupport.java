...
public abstract class JobStoreSupport implements JobStore, Constants {
    ...
    protected long getMisfireTime() {
        long misfireTime = DateTimeUtils.currentTimeMillis();
    ...
    private static long ftrCtr = DateTimeUtils.currentTimeMillis();
    ...
    protected long lastCheckin = DateTimeUtils.currentTimeMillis();
    ...
    protected List<SchedulerStateRecord> findFailedInstances(Connection conn)
        ...
        try {
            ...
            lastCheckin = DateTimeUtils.currentTimeMillis();
    ...
    protected long calcFailedIfAfter(SchedulerStateRecord rec) {
        return rec.getCheckinTimestamp() +
            Math.max(rec.getCheckinInterval(), 
                    (DateTimeUtils.currentTimeMillis() - lastCheckin)) +
            7500L;
    }
    ...
    protected void clusterRecover(Connection conn, List<SchedulerStateRecord> failedInstances)
        ...
        if (failedInstances.size() > 0) {
            ...
            long recoverIds = DateTimeUtils.currentTimeMillis();
        ...
        public void run() {
            while (!shutdown) {
                ...
                if (!shutdown) {
                    ...
                    long transpiredTime = (DateTimeUtils.currentTimeMillis() - lastCheckin);
            ...
            while (!shutdown) {
                ...
                if (!shutdown) {
                    ...
                    if (!recoverMisfiredJobsResult.hasMoreMisfiredTriggers()) {
                        timeToSleep = getMisfireThreshold() - (DateTimeUtils.currentTimeMillis() - sTime);
                        if (timeToSleep <= 0) {
                            timeToSleep = 50l;
                        }
